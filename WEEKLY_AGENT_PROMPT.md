...
### Style Compliance (Simple Rule: Use guide-template.html)

**Source of Truth:** `guide-template.html`

When updating or creating guides:
...
If YES to all three → ready to commit  
If NO to any → copy that section from template

**No Separate Checklist Needed.** The template IS the style guide.
...
- Check: Does it match guide-template.html structure?
...
- Start from guide-template.html, add content only
...
- ✓ No custom CSS or style duplication
...
- **QUICK CHECK:** `STYLE_GUIDE_CHECKER.md` (3-question checklist)
- **REFERENCE:** `STYLE_REFERENCE.md` (why we use templates, not checklists)
- Local structure: top-level directories = guide categories (orchestration, safety, evaluation, etc.)
- Each directory contains HTML guides based on guide-template.html
...
- Details: Brief summary of what changed (e.g., "Updated 2 guides, 10 current news items, 12 archived")